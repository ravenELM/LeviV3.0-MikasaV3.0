...
module.exports = {
  ...
  async run(m, { conn, args }) {
    ...
    if (!input) {
      return conn.sendMessage(chatId, { text: "❌ Please type a message for Levi." }, { quoted: m });
    }
    ...
    try {
      ...
      await conn.sendMessage(chatId, { text: `🤖 Levi: ${reply}` }, { quoted: m });
    } catch (error) {
      console.error("[Levi Chatbot] OpenAI API error:", error.response?.data || error.message);
      await conn.sendMessage(chatId, { text: "❌ Levi is currently unavailable." }, { quoted: m });
    }
  },
};
