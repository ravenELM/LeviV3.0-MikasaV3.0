...
module.exports = {
  name: 'deacteconomy',
  aliases: [],
  description: 'Deactivate economy in this group (owner only)',
  ...
  async run(msg, { conn, ownerIDs }) {
    ...
    if (!groups.includes(from)) {
      return conn.sendMessage(from, {
        text: '❌ This group is not activated for economy.'
      }, { quoted: msg });
    }

    groups = groups.filter(gid => gid !== from);
    fs.writeFileSync(file, JSON.stringify(groups, null, 2), 'utf8');

    return conn.sendMessage(from, {
      text: '✅ Economy deactivated for this group!'
    }, { quoted: msg });
  }
};
